...
def rgb2rawburst(image, burst_size, prepro, downsample_factor=4.0, burst_transformation_params=None,
    ...
    if use_smoothstep:
        image_o = invert_smoothstep(image)

    # Inverts gamma compression.
    if use_gamma:
        image_o = gamma_expansion(image_o)

    # Inverts color correction.
    image_o = apply_ccm(image_o, rgb2cam)

    # Approximately inverts white balance and brightening.
    image_o = safe_invert_gains(image_o, rgb_gain, red_gain, blue_gain)

    # Clip saturated pixels.
    image_o = image_o.clamp(0.0, 1.0)

    # Generate LR burst
    image_burst_rgb, kernel_burst, flow_vectors = single2lrburst(image, burst_size=burst_size, prepro=prepro,
                                                   downsample_factor=downsample_factor,
                                                   transformation_params=burst_transformation_params,
                                                   interpolation_type=interpolation_type)

    # tvutils.save_image(image_burst_rgb.data, f'tmp/image_t.png', normalize=True)
    # tvutils.save_image(image.data, f'tmp/image_hr.png', normalize=True)

    for i in range(len(image_burst_rgb)):
        # Approximately inverts global tone mapping.
        if use_smoothstep:
            image_burst_rgb[i] = invert_smoothstep(image_burst_rgb[i])

        # Inverts gamma compression.

        if use_gamma:
            image_burst_rgb[i] = gamma_expansion(image_burst_rgb[i])

        # Inverts color correction.
        image_burst_rgb[i] = apply_ccm(image_burst_rgb[i], rgb2cam)

        # Approximately inverts white balance and brightening.
        image_burst_rgb[i] = safe_invert_gains(image_burst_rgb[i], rgb_gain, red_gain, blue_gain)

        # Clip saturated pixels.
        image_burst_rgb[i] = image_burst_rgb[i].clamp(0.0, 1.0)
    ...
    return image_burst, image_o, kernel_burst, image_burst_rgb, flow_vectors, meta_info
...
def single2lrburst(image, burst_size, prepro, downsample_factor=4.0, transformation_params=None,
    ...
    for i in range(burst_size):
        ...
        # sample_pos_inv = sample_pos_inv.permute(2, 0, 1).contiguous()
        ...
        C, H, W = image_t.size()
        image_t, re_code, kernel = prepro(image_t.view(1, -1, H, W), kernel=True)

        burst.append(image_t.cpu())
        kernels.append(kernel.cpu())
        sample_pos_inv_all.append(sample_pos_inv / downsample_factor)

    burst_images = torch.cat(burst, dim=0)
    burst_kernels = torch.cat(kernels, dim=0)
    sample_pos_inv_all = torch.cat(sample_pos_inv_all, dim=0)
    ...
    return burst_images, burst_kernels, flow_vectors
