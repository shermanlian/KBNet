...
from data_processing.util import imresize
...
def random_batch_kernel(
    ...
    if rate_iso == 1:
        ...
        radians = np.random.uniform(0, np.pi, (batch))
...
class SRMDPreprocessing(object):
    def __init__(
        ...
        # "./pca_matrix/pca_matrix.pth", map_location=lambda storage, loc: storage
        "./pca_matrix/pca_aniso_matrix_x4.pth", map_location=lambda storage, loc: storage
    )
    ...
    def __call__(self, hr_tensor, kernel=False):
        ...
        lr_blured_t = self.blur(hr_var, b_kernels)
        ...
        # Down sample
        if self.scale != 1:
            lr_blured_t = b_Bicubic(lr_blured_t, self.scale)
        else:
