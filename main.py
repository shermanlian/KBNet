...
def main_worker(local_rank, nprocs, args):
    if checkpoint.ok:
        ...
        if nprocs > 1:
            ...
            train_loader = DataLoader(dataset=train_data, batch_size=batch_size, num_workers=32,
            ...
            valid_loader = DataLoader(dataset=valid_data, batch_size=batch_size, num_workers=16,
                                      pin_memory=False, drop_last=True, sampler=valid_sampler)
        else:
            train_sampler = None
            train_loader = DataLoader(dataset=train_data, batch_size=args.batch_size, num_workers=8,
                                    shuffle=True, pin_memory=False, drop_last=True)  # args.cpus
