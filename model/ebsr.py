...
class EBSR(nn.Module):
    def __init__(self, args):
        ...
        if self.lrcn == True:
            WARB = functools.partial(arch_util.WideActResBlock, nf=nf)
            LRCN = functools.partial(arch_util.LRSCWideActResGroup, n_resblocks=n_resblocks, nf=nf)
        else:
            WARB = functools.partial(arch_util.WideActResBlock, nf=nf)

        #### extract features (for each frame)
        self.conv_first = wn(nn.Conv2d(args.burst_channel, nf, 3, 1, 1, bias=True))
        self.feature_extraction = arch_util.make_layer(WARB, front_RBs)
        ...
        #### reconstruction
        if self.lrcn == False:
            self.recon_trunk = arch_util.make_layer(WARB, back_RBs)
        else:
            self.recon_trunk = nn.Sequential(
                arch_util.make_layer_idx(LRCN, back_RBs),
