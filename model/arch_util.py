...
class AdaptiveCARB(nn.Module):
    def __init__(self, nf=64, reduction=4, code_length=15):
        super(AdaptiveCARB, self).__init__()
        self.conv1 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
        self.conv2 = nn.Conv2d(nf, nf, 3, 1, 1, bias=True)
        self.avg_pool = nn.AdaptiveAvgPool2d(1)
        self.gamma = nn.Sequential(
            nn.Linear(nf+code_length, nf // reduction, bias=False),
            nn.ReLU(inplace=True),
            nn.Linear(nf // reduction, nf, bias=False),
            nn.Sigmoid()
        )
    ...
    def forward(self, input):
        ...
        b, c, h, w = out.size()
        g = self.avg_pool(out).view(b, c)
        y = torch.cat([g, kernel_code], dim=1)
        # print(y.shape)
        gamma = self.gamma(y).view(b, c, 1, 1)
        beta = self.beta(y).view(b, c, 1, 1)
        out = gamma * out + beta
        return identity + out, kernel_code
